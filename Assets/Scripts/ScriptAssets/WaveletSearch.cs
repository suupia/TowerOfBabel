...
public static class WaveletSearch
{
    ...
    // 7 * 2
    // 8 6 4 の優先順位で判定していく
    static Vector2Int[] orderInDirectionArrayRightUp1 = new Vector2Int[] { Vector2Int.right + Vector2Int.up, Vector2Int.right, Vector2Int.up, Vector2Int.right + Vector2Int.down, Vector2Int.left + Vector2Int.up, Vector2Int.down, Vector2Int.left, Vector2Int.left + Vector2Int.down };
    
    // 4 2 1
    // 6 * 3
    // 8 7 5 の優先順位で判定していく
    static Vector2Int[] orderInDirectionArrayRightUp2 = new Vector2Int[] { Vector2Int.right + Vector2Int.up, Vector2Int.up, Vector2Int.right, Vector2Int.left + Vector2Int.up, Vector2Int.right + Vector2Int.down, Vector2Int.left, Vector2Int.down, Vector2Int.left + Vector2Int.down };
    ...
    // 2 * 7
    // 4 6 8 の優先順位で判定していく
    static Vector2Int[] orderInDirectionArrayLeftUp1 = new Vector2Int[] { Vector2Int.left + Vector2Int.up, Vector2Int.left, Vector2Int.up, Vector2Int.left + Vector2Int.down, Vector2Int.right + Vector2Int.up, Vector2Int.down, Vector2Int.right, Vector2Int.right + Vector2Int.down };
    
    // 1 2 4
    // 3 * 6
    // 5 7 8 の優先順位で判定していく
    static Vector2Int[] orderInDirectionArrayLeftUp2 = new Vector2Int[] { Vector2Int.left + Vector2Int.up, Vector2Int.up, Vector2Int.left, Vector2Int.right + Vector2Int.up, Vector2Int.left + Vector2Int.down, Vector2Int.right, Vector2Int.down, Vector2Int.right + Vector2Int.down };
    ...
    // 2 * 7
    // 1 3 5 の優先順位で判定していく
    static Vector2Int[] orderInDirectionArrayLeftDown1 = new Vector2Int[] { Vector2Int.left + Vector2Int.down, Vector2Int.left, Vector2Int.down, Vector2Int.left + Vector2Int.up, Vector2Int.right + Vector2Int.down, Vector2Int.up, Vector2Int.right, Vector2Int.right + Vector2Int.up };
    
    // 5 7 8
    // 3 * 6
    // 1 2 4 の優先順位で判定していく
    static Vector2Int[] orderInDirectionArrayLeftDown2 = new Vector2Int[] { Vector2Int.left + Vector2Int.down, Vector2Int.down, Vector2Int.left, Vector2Int.right + Vector2Int.down, Vector2Int.left + Vector2Int.up, Vector2Int.right, Vector2Int.up, Vector2Int.right + Vector2Int.up };
    ...
    // 7 * 2
    // 5 3 1 の優先順位で判定していく
    static Vector2Int[] orderInDirectionArrayRightDown1 = new Vector2Int[] { Vector2Int.right + Vector2Int.down, Vector2Int.right, Vector2Int.down, Vector2Int.right + Vector2Int.up, Vector2Int.left + Vector2Int.down, Vector2Int.up, Vector2Int.left, Vector2Int.left + Vector2Int.up };

    // 8 7 5
    // 6 * 3
    // 4 2 1 の優先順位で判定していく
    static Vector2Int[] orderInDirectionArrayRightDown2 = new Vector2Int[] { Vector2Int.right + Vector2Int.down, Vector2Int.down, Vector2Int.right, Vector2Int.left + Vector2Int.down, Vector2Int.right + Vector2Int.up, Vector2Int.left, Vector2Int.up, Vector2Int.left + Vector2Int.up };

    public static List<Vector2Int> NonDiagonalSearchShortestRoute(Vector2Int startPos, Vector2Int endPos,OrderInDirection orderInDirection,int mapWidth, int mapHeight,int wallID, System.Func<Vector2Int, int, bool> InspectPosIsWallID) //PlaceNumAroundのとき4マス判定する
    {
        return _NonDiagonalSearchShortestRoute(startPos,endPos, orderInDirection,  mapWidth,  mapHeight,  wallID, InspectPosIsWallID);
    }
    public static List<Vector2Int> DiagonalSearchShortestRoute(Vector2Int startPos, Vector2Int endPos, OrderInDirection orderInDirection, int mapWidth, int mapHeight, int wallID, System.Func<Vector2Int, int, bool> InspectPosIsWallID) //PlaceNumAroundのとき8マス判定する
    {
        return ConvertToDiagonalRoute( _NonDiagonalSearchShortestRoute(startPos, endPos, orderInDirection, mapWidth, mapHeight, wallID, InspectPosIsWallID), wallID,InspectPosIsWallID);
    }

    public static bool isWithinTheAttackRange(Vector2Int gridPos, int attackRange, int targetID, out Vector2Int targetPos,int mapWidth, int mapHeight, System.Func<Vector2Int,int,bool> MapValueIsTargetID) //最も近い攻撃対象の座標を返す （存在しないときはVector2Int.zeroを返す）
    {
        ...
        for (int k = 0; k < maxRange; k++) //kは中心のマスから何マスまで歩けるかを表す
        {
            ...
            for (int j = 0; j < searchRangeArray.GetLength(0); j++)
            {
                for (int i = 0; i < searchRangeArray.GetLength(1); i++)
                {
                    ...
                    if (vector.x < 0 || vector.y < 0 || vector.x > mapWidth || vector.y > mapHeight)
                    {
                        continue;
                    }

                    if (searchRangeArray[i, j] == lookingForValue && MapValueIsTargetID(vector, targetID ))
                    {
    ...
    public static List<Vector2Int> SearchRandomDiagonalRouteForEnemy(Vector2Int startPos,Vector2Int endPos,OrderInDirection orderInDirection, int mapWidth,int mapHeight, int wallID,System.Func<Vector2Int, int, bool> InspectPosIsWallID)
    {
        //数字を置くとき周り8マスに置いていく
        int width = mapWidth;
        ...
        if (InspectPosIsWallID(startPos,wallID))
        {
        ...
        for (int y = 0; y < height; y++)
        {
            ...
            {
                if (InspectPosIsWallID(new Vector2Int(x,y),wallID))
                {
        ...
        shortestRouteList = DiagonalSearchShortestRoute(startPos, relayPosList[randRouteNum],orderInDirection, mapWidth, mapHeight, wallID, InspectPosIsWallID);
        shortestRouteList.RemoveAt(shortestRouteList.Count - 1); //連結するときに重複してしまうので削除する
        shortestRouteList.AddRange(DiagonalSearchShortestRoute(relayPosList[randRouteNum], endPos,orderInDirection, mapWidth, mapHeight, wallID, InspectPosIsWallID));
        ...
        bool isInTheSameGroup(Vector2Int pos1, Vector2Int pos2) //上下左右に同じ値のマスを移動してたどり着くことができる
        {
            ...
            if (GetValueFromVector(pos1) != GetValueFromVector(pos2)) Debug.LogError($"pos1:{pos1}とpos2:{pos2}の値が異なります");
        ...
        void PlaceFlagAroud(Vector2Int pos,int searchValue) //上下左右に同じ数字が入っていた場合にフラグを立てていく（再起関数的に呼ぶ）
        {
            ...
            for (int y = -1; y < 2; y++)
            {
                for (int x = -1; x < 2; x++)
                {
                    ...
                    if (GetValueFromVector(inspectPos) == searchValue && groupFlagMap[inspectPos.y,inspectPos.x] == false)
                    {
    ...
    //以下、private関数
    private static List<Vector2Int> _NonDiagonalSearchShortestRoute(Vector2Int startPos, Vector2Int endPos, OrderInDirection orderInDirection, int mapWidth, int mapHeight, int wallID,System.Func<Vector2Int, int, bool> InspectPosIsWallID)
    {
        int[] _values = null;  //2次元配列のように扱う
        int _initiValue = -10; //PlaceNumAroundで重複して数字を置かないようにするために必要
        int _wallValue = -1;   //wallのマス
        ...
        Vector2Int[] orderInDirectionArray; //StoreShortestRouteを実行するごとにorderInDirectionArray1とorderInDirectionArray2を入れ替える
        Vector2Int[] orderInDirectionArray1;
        Vector2Int[] orderInDirectionArray2;
        ...
        if (InspectPosIsWallID(startPos,wallID))
        {
            Debug.LogError("SearchShortestRouteのstatPosにwallIDが含まれています");
            return null;
        }
        if (InspectPosIsWallID(endPos,wallID))
        {
        ...
        //初期化
        _values = new int[mapWidth * mapHeight];
        shortestRouteList = new List<Vector2Int>();
        FillAll(_initiValue); //mapの初期化は_initiValueで行う
        switch (orderInDirection)
        {
            case OrderInDirection.RightUp:
                orderInDirectionArray1 = orderInDirectionArrayRightUp1;
                orderInDirectionArray2 = orderInDirectionArrayRightUp2;
                break;
            case OrderInDirection.LeftUp:
                orderInDirectionArray1 = orderInDirectionArrayLeftUp1;
                orderInDirectionArray2 = orderInDirectionArrayLeftUp2;
                break;
            case OrderInDirection.LeftDown:
                orderInDirectionArray1 = orderInDirectionArrayLeftDown1;
                orderInDirectionArray2 = orderInDirectionArrayLeftDown2;
                break;
            case OrderInDirection.RightDown:
                orderInDirectionArray1 = orderInDirectionArrayRightDown1;
                orderInDirectionArray2 = orderInDirectionArrayRightDown2;
                break;
            default: //デフォルトは右上
                orderInDirectionArray1 = orderInDirectionArrayRightUp1;
                orderInDirectionArray2 = orderInDirectionArrayRightUp2;
        ...
        bool IsOutOfRange(int x, int y)
        {
            if (x < -1 || x > mapWidth) { return true; }
            if (y < -1 || y > mapHeight) { return true; }
    ...
    static int[,] CalcSearchRangeArray(int advancingDistance, int lookingForValue, int notLookingForValue, int centerValue) //中心からadvancingDstanceだけ進んで（斜め移動はなし）隣接できる外側のマスをlookinForValueにした2次元配列を返す　lookinForValueはドーナッツ状に入る
    {
    ...
    static List<Vector2Int> ConvertToDiagonalRoute(List<Vector2Int> nonDiagonalRoute, int wallID, System.Func<Vector2Int, int, bool> InspectPosIsWallID)
    {
        ...
        for (int i = 0; i < nonDiagonalRoute.Count; i++)
        {
            ...
            if ((((nextPos - gridPos).x == 0 && (nextNextPos - nextPos).y == 0) || ((nextPos - gridPos).y == 0 && (nextNextPos - nextPos).x == 0)) && CanMoveDiagonally(gridPos, nextNextPos,wallID,InspectPosIsWallID))
            {
                if (removeIndex.Count != 0 && removeIndex[removeIndex.Count - 1] + 1 == i + 1) continue; //除く点が連続してはいけない
                removeIndex.Add(i + 1);
            }
        ...
        {
            nonDiagonalRoute.RemoveAt(removeIndex[removeIndex.Count - 1 -i]); //removeIndexが昇順に数字が入っていることを利用する。nonDiagonalRouteは添え字が大きいほうから削除していく
        }
        ...
        //以下ローカル関数
        bool CanMoveDiagonally(Vector2Int prePos, Vector2Int afterPos, int wallID, System.Func<Vector2Int, int, bool> InspectPosIsWallID)
        {
            Vector2Int directionVector = afterPos - prePos;
            ...
            {
                //水平方向の判定
                    if(InspectPosIsWallID(new Vector2Int(prePos.x + directionVector.x, prePos.y),wallID))
                {
                    return false;
                }

                //垂直方向の判定
                    if (InspectPosIsWallID(new Vector2Int(prePos.x, prePos.y + directionVector.y), wallID))
